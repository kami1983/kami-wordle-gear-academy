...
use io::{Action, Event, GameState, GameStatus, SessionStatus, StateQuery};
...
fn test() {
    ...
    let target_program = Program::from_file(&system, "../wordle-program/target/wasm32-unknown-unknown/debug/kami_wordle_gear_academy.opt.wasm");
    ...
    let state: GameState = wordle_program.read_state(StateQuery::GetGameState).expect("Unexpected invalid state.");
